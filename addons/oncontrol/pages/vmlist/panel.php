...
if ($snapshotList->response['0'] == '') {
    unset ($snapshotList->response['0']);
}
...
$vm_data = $this->onconnect->getVmData($machineInfo->vmid);
...
switch ($status->response) {
    ...
    default :
        ...
        if ($status->response == 'SAVE_SUSPEND' or $status->response == 'BOOT_SUSPENDED') {
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="col-sm-12">
                                <b><?=$LANG['Status']?>:</b> <b id="state"
                                                                style="font-size: 110%; border: 1px solid <?php echo get_color_by_status($status->response) ?>; color: <?php echo get_color_by_status($status->response); ?>;">
                                    <?= $vm_data->response->STATE ?>
                                </b>
                            </div>
                            <div class="col-sm-12">
                                <b><?=$LANG['IP']?>:</b> <?= $vm_data->response->IP ?>
                            </div>
                            <div class="col-sm-12">
                                <b><?=$LANG['VMID']?></b> <a
                            ...
                            <div class="col-sm-12">
                                <b><?=$LANG['PanelHeading']?>:</b><?= $vm_data->response->HOST; ?>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="col-sm-12">
                            ...
                            <div class="col-sm-12">
                                <b><?=$LANG['vcpu']?>:</b> <?= $vm_data->response->CPU; ?>
                            </div>
                            <div class="col-sm-12">
                                <b><?=$LANG['ram']?>:</b> <?= $vm_data->response->RAM; ?>MB
                            </div>
                            <div class="col-sm-12">
                                <b><?=$LANG['import']?>:</b> <?= $vm_data->response->IMPORTED ?>
                            </div>
                        </div>
                        <div class="col-sm-12">
...
<!--                </div>-->
                ...
                <div class="col-sm-12 text-center" style="margin-top: 20px;">
                    <button <?= $vm_data->response->STATE != 'RUNNING' ? 'disabled' : ''; ?>
                            data-toggle="collapse" data-target="#collapse1" class="btn btn-block btn-default">
                        Snapshots(<?= $vm_data->response->STATE != 'RUNNING' ? ''.$LANG["tariff"].'' : ''; ?><?=$LANG['open']?>)
                    </button>

                    <div id="collapse1" class="collapse">
                        <ul class="list-group">
                            <?php foreach ($snapshotList->response as $snapshot): ?>
